...
<script type="text/javascript">
	$(document).ready(
			function() {
				$('#j_upload_ticket_no_file_info').click(function() {
				...
				$('.g-btn_tri').click(
						function() {
							...
							showAlert("确认要触发查询操作吗？", 'confirm', function() {
								$.get('../ticket/ticket-ctrl!trigger.do?id='
										+ p+ '&v='
									+ Math.round(Math.random() * 100000), function(data) {
									showAlert(data);
									$('#' + p + 'Q').html('执行中');
								});
							});
				...
				$('.g-btn-detail').click(
						function() {
							p = $(this).attr('name');
							//console.log(p);
							$.get('../ticket/ticket-ctrl!detail.do?id=' + p
									+ '&v='
...
<body style="margin: 10px">
	...
	<table class="g-table order-table">
		...
		<tr>
			<td><a href="${ctx}/jsp/build/models/ImpTicketNo.xls"
				style="display:inline-block;" class="pur_btn_css">模版下载</a></td>
		</tr>

		<tr>
			<td>
				<form action="${ctx }/ticket/ticket-ctrl!imptickets.do"
					...
					<input class="g-btn" type="file" name="file"
						...
						id="j_upload_ticket_no_file_info" value="上传" />
				</form>
			</td>
		</tr>
	</table>
	...
	<form action="${ctx }/ticket/ticket-ctrl!querylist.do" name="form"
		method="post">
					...
					<tr>
						...
						<td><a class="g-btn-detail" href="javascript:void(0);"
							name="${t.id }" id="${t.id }">详情</a> <c:if
								...
								<a class="g-btn_tri" href="javascript:void(0);" name="${t.id }"
									id="${t.id }Q">已完成</a>
							</c:if> <a class="g-btn-down" href="javascript:void(0);"
							name="${t.fileno }" id="${t.fileno }">下载</a><br>
							${t.createTime }</td>
					</tr>
				</c:forEach>
				<tr>
					<td colspan="8"><p class="page">
							<p:page form="form"
								action="${ctx }/ticket/ticket-ctrl!querylist.do" />
						</p></td>
				</tr>
			</tbody>
		</table>
		<input type="hidden" value="imp_tickets" name="page_name"
			id="page_name" />
	</form>
</body>
</html>
