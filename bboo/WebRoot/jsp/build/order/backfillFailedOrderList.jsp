...
<%
	request.getSession().setAttribute("startPage", "backfillFailedOrderList");
%>
<!DOCTYPE html>
<html>
<head>
...
queryOrderSummary=function(data){
		...
		data.backno='3';  //3表示回填失败的订单,回填失败有可能是调用分销商平台接口出错了,也有可能是rt3次了,没有获取到票号相关信息.
		$.ajax({
...
// 							var td_arrAircode="<td>"+item.arrAircode+"</td>"; //到达城市三字码
							...
							if(item.flights){
								$.each(item.flights,function(j,flight){
									td_lowestPrice+="<span style='font-size:18px'>"+flight.cabin+":"+flight.cabinNum+";</span>"; //仓位+舱位剩余数
...
<%--							var td_slfStatus=""+getSlfStatusCH(item.slfStatus)+"</td>"; //订单状态//这个字段暂时不确定--%>
							...
							var var_tr=td_orderNo+td_pnrCode+td_allprice+td_passengerCount+td_dep_arr+td_flightDate_flightNo+td_lowestPrice+td_pnrNoTime+td_time+td_remark+td_flightType+td_lockUser;
							...
							var_a=var_a+"<br/>"+"<a href='${ctx }/order/gj-order-detail!queryOrderOutByID.do?id="+item.id+"&lockUser="+currentLockUser+"&operate=orderOutDeal'>回填票号</a>";
							if(item.backno!=1){  //如果订单回填票号的状态还不是回填成功,其实这个if不需要,因为查询出来的都是回填失败的,backno=3固定死了
								var_a=var_a+"<br/>"+"<a href='javascript:alreadyBackfillOrderById(\""+item.id+"\")'>已回填</a>";
							}
...
function alreadyBackfillOrderById(id){
	$.ajax({
		type : "post",
		dataType : "json",
		data : {
		...
		url : "${ctx }/order/gj-order-detail!alreadyBackfillOrderById.do",
		success : function(result) {
			if (result.error) {
				alert(result.error);
				...
				queryOrderSummary(data);  //更新成功后刷新页面
			}
		},
		error : function(XMLHttpRequest, textStatus, errorThrown) {
			alert("请求出错了");
